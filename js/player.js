...
function repairPlayerData(data){
    ...
    if(!fixed.headquarters || typeof fixed.headquarters !== "object") fixed.headquarters = { level: 1 };
    if(typeof fixed.headquarters.level !== "number" || !Number.isFinite(fixed.headquarters.level)) fixed.headquarters.level = 1;
    fixed.headquarters.level = Math.max(1, Math.min(50, Math.floor(fixed.headquarters.level)));
    numbers.forEach(key=>{
    ...
    // תיקון הגנתי: אם maxEnergy שמור גבוה בהרבה ממה שהנוסחה
    // (100 * 1.05^(level-1)) הייתה נותנת לרמה הזו - כנראה נתון פגום
    // משמירה ישנה, ומתקנים אותו לערך תקין לפי הרמה.
    const expectedMaxEnergy =
        Math.floor(100 * Math.pow(1.05, Math.max(0, (fixed.level||1) - 1)));
    if(fixed.maxEnergy > expectedMaxEnergy * 3){
        fixed.maxEnergy = expectedMaxEnergy;
        if(fixed.energy > fixed.maxEnergy){
            fixed.energy = fixed.maxEnergy;
        }
    }
    return fixed;
}
